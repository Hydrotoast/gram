...
trait Tensor[@sp A] {
  ...
  /** Given an index and a value, replaces the element at the given index with
    * the given value.
    *
    * @param index The index.
  ...
  /** Given a subscript and a value, replaces the element at the given subscript
  ...
  override def equals(o: Any): Boolean =
    o match {
      case that: Tensor[A] =>
        ...
        while (it1.hasNext && it2.hasNext) if (it1.next != it2.next)
          return false
