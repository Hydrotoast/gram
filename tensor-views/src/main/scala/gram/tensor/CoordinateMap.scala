...
private[tensor] sealed trait CoordinateMap {
  ...
  /** The point that this function maps to if it is a singleton. The behavior is
    * undefined if this range is not a singleton.
    */
  def singleton: Int

  /** Given an element of the domain, returns the corresponding mapping. The
    * behavior is undefined if the element outside the domain [0, domainSize).
    *
...
/** A linear function from [0, end) to [0, end).
  ...
  */
private[tensor] final case class ZeroTo(end: Int) extends CoordinateMap {
  require(end > 0, s"end must be positive to produce a valid range: end=$end")

  def domainSize: Int = end
  def singleton: Int = end - 1
  def map(x: Int): Int = x
}
...
/** An affine function from [0, domainSize) to [start, end).
  ...
  * @param end   The exclusive end of the range.
  */
private[tensor] final case class Slice(start: Int, end: Int)
    extends CoordinateMap {
  require(
    end > start,
    s"end should be greater than start i.e. end > start: $end > $start")

  def domainSize: Int = end - start
  def singleton: Int = start
  def map(x: Int): Int = start + x
}
...
private[tensor] final case class Step(
  ...
  def domainSize: Int = (end - start) / step
  def singleton: Int = start
  def map(x: Int): Int = start + x * step
}
...
/** A mapping to a single point. This is always a singleton range.
  ...
  */
private[tensor] final case class Point(singleton: Int) extends CoordinateMap {
  require(
    singleton >= 0,
    s"point should be non-negative to be valid: point=$singleton")

  def domainSize: Int = 1
  override def isConstant: Boolean = true
  def map(x: Int): Int = throw new UnsupportedOperationException
}
